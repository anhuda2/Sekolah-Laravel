                    ...
                    @foreach ($class->students as $student)
                        <div class="flex items-center justify-between border border-gray-200">
                            ...
                            <div class="w-5/12 text-sm text-right py-2 px-4 flex items-center justify-end">
                                ...
                                <label class="ml-4 block text-red-500 font-semibold sm:border-r sm:pr-4">
                                    <input name="attendences[{{ $student->id }}]" class="leading-tight" type="radio" value="absen">
                                    <span class="text-sm">Alfa</span>
                                </label>                                
                                <label class="ml-4 block text-yellow-500 font-semibold sm:border-r sm:pr-4">
                                    <input name="attendences[{{ $student->id }}]" class="leading-tight" type="radio" value="sakit">
                                    <span class="text-sm">Sakit</span>
                                </label>
                                <label class="ml-4 block text-blue-500 font-semibold sm:border-r sm:pr-4">
                                    <input name="attendences[{{ $student->id }}]" class="leading-tight" type="radio" value="hadir">
                            ...
                            <input type="hidden" name="teacher_id" value="{{ $class->teacher_id }}"><input type="hidden" name="teacher_id" value="{{ $class->teacher_id }}">
                        </div>
                    @endforeach
                    <div class="mt-6">
